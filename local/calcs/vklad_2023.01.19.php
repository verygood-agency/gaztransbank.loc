...
if(!empty($calcArray)) {
    ...
    <script>
        ...
        function updateCalc(status)
        {
            ...
            // Формируем массив с подходящей валютой и определяем значения сроков
            calcArray.forEach(function(item, i) {
                if (item['UF_CURRENCY'] == valCurrency && item['UF_START_SUM'] && item['UF_END_SUM'] && item['UF_CURRENCY'] && item['UF_DAYS'] && item['UF_PERCENT']) {
                    ...
                    arrayCurrency.push(item);
                };
            });

            // Формируем массив с подходящим сроком и определяем минимальные и максимальные значения
            if (!valDay) {valDay = day['1']}
            ...
            arrayDay.forEach(function(item, i) {
