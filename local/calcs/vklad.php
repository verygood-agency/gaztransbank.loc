    ...
    function updateCalc(status)
    {
      ...
      arrayItog.forEach(function(item, i) {
        if (parseInt(valSum) >= parseInt(item['UF_START_SUM']) && parseInt(valSum) <= parseInt(item['UF_END_SUM'])) {
          ...
          percent2 = item['UF_PERCENT2'];
          percent2Sum = item['UF_PERCENT2_SUM'];
        };
      });
      ...
      //Расчёт дохода по 2 проценту при превышении суммы
      if (percent2 != null && percent2Sum != null && parseInt(sumCalc) > parseInt(percent2Sum)) {
        sumCalc = sumCalc - percent2Sum;

        if (calcType == 'Ежемесячная капитализация') {
          income += Math.round((parseInt(sumCalc) * Math.pow((1 + ((parseFloat(percent2) / 100) / 12)), (parseInt(valDay) / (parseInt(daysYear)/12)))) - parseInt(sumCalc));
        } else {
          income += Math.round((parseInt(sumCalc) * parseFloat(percent2) * parseInt(valDay) / parseInt(daysYear)) / 100);
        }
        console.log(income)
        $('.itogPercent').html(percent + '% <br>' + percent2 + '%');
        sumCalc = percent2Sum;
      }

      //Расчёт дохода по 1 проценту
      if (calcType == 'Ежемесячная капитализация') {
        income += Math.round((parseInt(sumCalc) * Math.pow((1 + ((parseFloat(percent) / 100) / 12)), (parseInt(valDay) / (parseInt(daysYear)/12)))) - parseInt(sumCalc));
      } else {
        income += Math.round((parseInt(sumCalc) * parseFloat(percent) * parseInt(valDay) / parseInt(daysYear)) / 100);
      }
      console.log(income);

      //Расчёт дохода для доп взноса
      if (dopOn && valDop) {
        if (calcType == 'Ежемесячная капитализация') {
